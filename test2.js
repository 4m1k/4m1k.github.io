(function(){
  ...
  // =========================================================================
  //  Часть A. Полноценный объект KP_PLUGIN, регистрируемый в Lampa.Api.sources
  // =========================================================================
    ...
    window.KP_PLUGIN = (function(){
      ...
      function convertElem(elem){
        ...
        var result = {
          ...
          production_countries: (elem.countries||[]).map(function(e){
        ...
        if(elem.sequels_obj){
          ...
          result.collection = {
            results: sequels.map(convertElem)
          };
        }
        ...
        if(elem.similars_obj){
          ...
          result.simular = {
            results: similars.map(convertElem)
          };
        }
  ...
  // =========================================================================
  //  Часть B. Регистрация плагина KP_PLUGIN в Lampa
  // =========================================================================
  ...
  // =========================================================================
  //  Часть C. Добавляем кнопку «Кинопоиск» в главное меню, 
  //           но вместо Select.show(...) открываем кастомное Activity
  // =========================================================================
  ...
  // При нажатии — переходим в наше кастомное меню
  ...
  // =========================================================================
  //  Часть D. Объявляем «component: kp_custom_menu» — 
  //           страницу-Activity со списком категорий (как в настройках).
  // =========================================================================

  // Список пунктов (как в Select.show)
  var KP_MENU_ITEMS = [
  ...
  // Регистрируем новый компонент через Lampa.Component
  Lampa.Component.add({
    name:'kp_custom_menu',
    ...
    onCreate(){
      const _this = this;
      // создаём прокрутку
      _this.scroll = new Lampa.Scroll({ mask:true, over:true });
      _this.scroll.render().addClass('menu-list');

      // Для каждого пункта создаём DOM-элемент
      KP_MENU_ITEMS.forEach(function(itemData){
          ...
          <div class="settings-param selector">
            <div class="settings-param__name">
              <div class="settings-folder" style="padding:0!important; display:flex; align-items:center;">
                ...
                <!-- Текст в классическом стиле Lampa -->
        ...
        // При клике — переходим в category_full
    ...
    onStart(){
      // при старте показываем наш контент
      this.activityLoader();
    },

    render(){
      // используем шаблон activity
      let view = Lampa.Template.js('activity');
      view.find('.activity__title').text('Кинопоиск');
      view.find('.activity__body').append(this.scroll.render());
      return view;
    },
