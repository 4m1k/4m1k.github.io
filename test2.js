  ...
  try {
    ...
    function setCache(key, value) {
      ...
      if(Object.keys(cache).length >= CACHE_SIZE){
        ...
        if(Object.keys(cache).length >= CACHE_SIZE){
          const times = Object.values(cache).map(e => e.timestamp).sort((a, b) => a - b);
        ...
        distributions.forEach(d => {
          if(d.date && (d.type === 'WORLD_PREMIER' || d.type === 'ALL')){
            ...
            if(!isNaN(timestamp) && (min === null || timestamp < min) &&
               (isNaN(yearTimestamp) || timestamp >= yearTimestamp)){
              min = timestamp;
              firstAirDate = d.date;
            }
          }
    ...
    // -------------------- Объединяем объект плагина для Lampa --------------------
    const KP_PLUGIN = {
      SOURCE_NAME,
      SOURCE_TITLE,
      main,
      ...
      clear: clearCache,
      person,
      seasons: seasons, // если функция seasons не нужна, можно вернуть пустой результат
      menuCategory: (params, onComplite) => onComplite([]),
      discovery,
      search
    };
    ...
    function startPlugin() {
      ...
      function addPlugin() {
        ...
        Object.defineProperty(Lampa.Api.sources, SOURCE_NAME, { get: () => KP_PLUGIN });
        let sources;
    ...
    const KP_PLUGIN = {
      ...
      clear: clearCache,
      person,
      seasons: seasons, // если функция seasons не нужна, можно вернуть пустой массив
      menuCategory: (params, onComplite) => onComplite([]),
      discovery,
      search
    };
