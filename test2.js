...
/***************************************************************************
 * Часть 4. Создаём компонент в настройках Lampa для пунктов «Кинопоиск»
 **************************************************************************/
(function addKpSettingsComponent() {
  // 1) Регистрируем новый компонент настроек
  Lampa.SettingsApi.addComponent({
    component: 'kp_plugin_menu',           // уникальное имя компонента
    name: 'Категории Кинопоиска',          // как будет называться в списке
    // Можем использовать ту же иконку, что и у кнопки «Кинопоиск»
    // или вставить свою:
    icon: `
      <svg
        xmlns="http://www.w3.org/2000/svg"
  ...
  // Функция-хелпер для скрытия "значений" (чтобы не было выпадающих списков)
  function hideValue(item) {
    // скрываем .settings-param__value, если хотим статический пункт
    $('.settings-param__value', item).hide();
  }

  // 2) Добавляем пункты как статические, каждый — через addParam
  // Пример для «Популярные фильмы»
  Lampa.SettingsApi.addParam({
    component: 'kp_plugin_menu',
    param: {
      name: 'kp_popular_films',
      type: 'static', // статический пункт
      default: true
    },
    field: {
      // HTML-код с нужной иконкой и классом `menu__text`
      name: `
        <div class="settings-folder" style="padding:0!important; display: flex; align-items: center;">
          ...
          <!-- Текст со шрифтом Lampa -->
          <div class="menu__text">Популярные фильмы</div>
        </div>
      `,
      description: 'Пример пункта «Популярные фильмы»'
    },
    onRender: function (item) {
      hideValue(item);
      // При клике «enter» можно сделать переход
      item.on('hover:enter', function () {
        // Например, сразу пушим Activity
        Lampa.Activity.push({
  ...
  // 3) Аналогично добавляем пункт «Топ Фильмы»
  Lampa.SettingsApi.addParam({
    component: 'kp_plugin_menu',
    param: {
      name: 'kp_top_films',
    ...
    field: {
      name: `
        <div class="settings-folder" style="padding:0!important; display: flex; align-items:center;">
          ...
          <div class="menu__text">Топ Фильмы</div>
        </div>
      `,
      description: ''
    },
  ...
  // 4) По аналогии добавьте остальные пункты: «Популярные российские фильмы», «Популярные сериалы» и т.д.
  //    С разными url: 'api/v2.2/films?order=NUM_VOTE&countries=34&type=FILM' и т.п.
