(function() {
  ...
  function component(object) {
    ...
    function account(url) {
      ...
      // Здесь вместо использования Lampa.Storage.get('token') используем фиксированное значение:
    ...
    // Остальные функции плагина (rch, externalids, updateBalanser, changeBalanser, requestParams, getLastChoiceBalanser, startSource, lifeSource, createSource, create, search, find, request, parseJsonDate, getFileUrl, toPlayElement, appendAPN, setDefaultQuality, display, parse, similars, getChoice, saveChoice, replaceChoice, clearImages, reset, loading, filter, selected, getEpisodes, watched, updateWatched, draw, contextMenu, empty, noConnectToServer, doesNotAnswer, getLastEpisode, start, render, back, pause, stop, destroy) остаются без изменений и взяты из исходного файла.
    // Для краткости полный текст не повторяется здесь – смотрите оригинальный файл akteronline.txt :contentReference[oaicite:0]{index=0}.
