(function () {
    ...
    var nativeFetch = (typeof window !== 'undefined' && typeof window.fetch === 'function')
    ...
    function isMirrorTmdb(url) {
        return typeof url === 'string'
            && (url.indexOf('apitmdb.') !== -1 || url.indexOf('tmdb.') !== -1)
            && url.indexOf(TMDB_HOST) === -1;
    }

    // Резервный URL для дозагрузки заблокированных карточек.
    // Если прокси Lampa включён — берём через abmsx, иначе напрямую.
    ...
    var cardCache = {}, imagesCache = {}, seasonCache = {};
    ...
    XMLHttpRequest.prototype.send = function () {
        ...
        function rerouteBlocked(respUrl, doneCb) {
            ...
            blockedCards[type + '*' + id] = true;

            if (sub === 'images') {
                fetchImages(id, type).then(function (data) {
                    patchXhr(xhr, data, null); doneCb();
                }, function () {
                    patchXhr(xhr, { id: parseInt(id, 10), logos: [], backdrops: [], posters: [] }, null); doneCb();
                });
        ...
        function handleBlocked() {
            ...
            return rerouteBlocked(respUrl, function () {
        ...
        xhr.onload = function () {
            if (!handled) { if (!handleBlocked()) { if (origOnLoad) origOnLoad.call(xhr); } }
        };
        xhr.onerror = function () {
            ...
            rerouteBlocked(reqUrl, function () {
                if (origOnReady) origOnReady.call(xhr);
                if (origOnLoad)  origOnLoad.call(xhr);
            });
        };
    ...
    function start() {
        ...
        // Прячем подменю прокси в настройках TMDB
