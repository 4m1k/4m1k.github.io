    ...
    if (xhr.status === 200) {
        ...
        switch (xhr.status) {
            case 400:
                ...
                Lampa.Noty.show('Неверный запрос.');
                break;
            case 401:
                console.error('Ошибка: Требуется авторизация.');
                Lampa.Noty.show('Требуется авторизация.');
                break;
            case 403:
                console.error('Ошибка: Доступ запрещен.');
                Lampa.Noty.show('Доступ запрещен.');
                break;
            case 404:
                console.error('Ошибка: Страница не найдена.');
                Lampa.Noty.show('Сервер не найден.');
                break;
            case 500:
                console.error('Ошибка: Внутренняя ошибка сервера.');
                Lampa.Noty.show('Внутренняя ошибка сервера.');
                break;
            default:
                console.error('Ошибка: Неизвестный статус.', xhr.status);
...
xhr.onerror = function () {
    console.error('Ошибка соединения:', xhr.status);
    Lampa.Noty.show('Не удалось подключиться к серверу.');
};
